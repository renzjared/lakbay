...
export function switchView(viewName) {
  ...
  document.querySelectorAll('.trip-nav-btn').forEach(btn => {
    btn.classList.remove('text-[#1cb0f6]'); btn.classList.add('text-gray-400');
    if (btn.dataset.target === viewName) { btn.classList.remove('text-gray-400'); btn.classList.add('text-[#1cb0f6]'); }
  });
...
async function renderTripList() {
  ...
  if (useSupabase && !currentUser) {
    list.innerHTML = `<div class="text-center text-gray-400 mt-12 font-bold bg-white p-8 rounded-2xl border-2 border-dashed border-gray-200"><div class="text-4xl mb-4">🔒</div><p>Please log in via Discord to view or create trips.</p></div>`;
    return;
  }
  ...
  trips.forEach(trip => {
    ...
    card.className = 'trip-card p-5 flex items-center justify-between mb-3';
    ...
    card.innerHTML = `
      <div class="flex items-center gap-4">
        ...
        <div>
          <h3 class="font-extrabold text-xl text-[#4b4b4b]">${trip.destination}</h3>
          <p class="text-sm font-bold text-gray-400 mt-1">${trip.days.length} Days Planned</p>
        </div>
      </div>
    ...
    card.addEventListener('click', async (e) => {
      ...
      playClick(); switchView('days'); const loadedTrip = await getTrip(trip.id); loadTripDays(loadedTrip);
    });
